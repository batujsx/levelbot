...
module.exports = async (client) => {
  ...
  client.on("messageCreate", async message => {
    ...
    if (xpp >= gerekli) {
      ...
      if (channel) {
      client.guilds.cache.get(message.guild.id).channels.cache.get(channel).send({content:`${message.author} hayırlı olsun dostum yeni levelin **${level + 1}.** böyle devam et ödüllerimiz var.`});
      } else {
      message.channel.send({content:`hayırlı olsun dostum yeni levelin **${level + 1}.** böyle devam et ödüllerimiz var.`});
      };
